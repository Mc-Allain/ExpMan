...
public class ComponentManager {
    ...
    public void hideInputErrors() {
        ...
        for (EditText errorEditText : errorEditTextList)
            errorEditText.setBackground(
                    ResourcesCompat.getDrawable(context.getResources(), R.drawable.input_bg_primary, null)
            );
    }
    ...
    public void showInputError(TextView textView, String string, EditText targetEditText) {
        ...
        targetEditText.setBackground(
                ResourcesCompat.getDrawable(context.getResources(), R.drawable.input_bg_error, null)
        );
    }
    ...
    public void checkInput(String string, boolean isRequired, String fieldName,
                           TextView errorTextView, EditText inputEditText) {
        hideInputError(errorTextView, inputEditText);

        if (Credentials.isEmpty(string) && isRequired) {
            showInputError(errorTextView,
            ...
            showInputError(errorTextView,
                    Credentials.getSentenceCase(
                            context.getString(R.string.length_error, "",
                                    Credentials.REQUIRED_LABEL_LENGTH)
                    ),
                    inputEditText);
        }
    }

    public void checkInput(String string, boolean isRequired, String fieldName, TextView errorTextView) {
        hideInputError(errorTextView);

        if (Credentials.isEmpty(string) && isRequired) {
            showInputError(errorTextView, context.getString(R.string.required_input_error, fieldName));
        } else if (!Credentials.isValidLength(string, Credentials.REQUIRED_LABEL_LENGTH, 0)) {
            showInputError(errorTextView,
                    Credentials.getSentenceCase(
                            context.getString(R.string.length_error, "",
    ...
    public void checkDate(String string, boolean isRequired, String fieldName, TextView errorTextView) {
        hideInputError(errorTextView);
    ...
    public void setPrimaryButtonEnabled(Button button, boolean isEnabled) {
        ...
        if (isEnabled) {
            button.setBackgroundColor(context.getResources().getColor(R.color.blue_primary));
            button.setTextColor(context.getResources().getColor(R.color.white));
        } else {
            button.setBackgroundColor(context.getResources().getColor(R.color.gray));
            button.setTextColor(context.getResources().getColor(R.color.system_darker_gray));
        }
    }
